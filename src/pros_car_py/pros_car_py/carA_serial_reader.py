...
from pros_car_py.pros_car_py.car_models import TwoWheelAndServoControlSignal
import rclpy
...
class CarAControlSubscriber(Node):
    ...
    def process_control_data(self, data):
        ...
        control_signal = TwoWheelAndServoControlSignal(**data)

        self._serial.write(orjson.dumps(control_signal,option=orjson.OPT_APPEND_NEWLINE))
        self.get_logger().info(f'Received direction: {direction}, target velocities: {target_vel}')
