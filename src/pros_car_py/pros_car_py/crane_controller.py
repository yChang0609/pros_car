...
class CraneController():
    def __init__(self, ros_communicator, data_processor, ik_solver, num_joints = 7):
        # initail pybullet
    ...
    # 定義每個軸的最小和最大角度
        ...
        if index == 99:
            if key == 'i':
                ...
                self.crane_state = [0, 0]
            self.update_crane_action(self.crane_state)
        # 獲取當前 joint 的限制
        elif 0 <= index < len(joint_limits):
    ...
    def update_action(self, joint_pos):
        ...
        print(f"update_action: {joint_pos}")
        self.ros_communicator.publish_robot_arm_angle(joint_pos)
    
    def update_crane_action(self, crane_state):
        print(f"update_crane_action: {crane_state}")
        self.ros_communicator.publish_crane_state(crane_state)
