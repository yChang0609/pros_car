...
class Nav2Processing:
    ...
    def get_action_from_nav2_plan_no_dynamic(self, goal_coordinates=None):
        if goal_coordinates is not None:
            self.ros_communicator.publish_goal_pose(goal_coordinates)
        ...
        if self.finishFlag == False:
            ...
            # self.index_length = len(self.global_plan_msg.poses)

        if not self.global_plan_msg:
            action_key = "STOP"
        else:
            try:
                z, w = (
                    self.global_plan_msg.poses[self.index].pose.orientation.z,
                    self.global_plan_msg.poses[self.index].pose.orientation.w,
                )
                plan_yaw = get_yaw_from_quaternion(z, w)
                car_position, car_orientation = (
                    self.data_processor.get_processed_amcl_pose()
